...
class InferenceBase(Base):
    ...
    def _create_dataset(self, user_tweet_df: pd.DataFrame):
        ...
        inference_set = InferenceDataset(
            ...
            inference_numbers,
            Variable(self._word_to_tensor(user_tweet_df['screen_name'][0]), requires_grad=False).unsqueeze(0).unsqueeze(1),
            Variable(self._word_to_tensor(user_tweet_df['u_name'][0]), requires_grad=False).unsqueeze(0).unsqueeze(1),
            Variable(self._desc_to_tensor(user_tweet_df['u_description'][0]), requires_grad=False).unsqueeze(0).unsqueeze(1),
